 ...
 * The input file should contain one IP address per line. Wildcards are
 * represented by '*'. For IPv4, each octet may be a number (0-255) or '*'.
 * Example lines: 10.0.*.* 192.168.1.* 2001:db8::*
...
static int parse_ipv6(const char *line, struct ipv6_mask *out)
{
	...
	while (token && group < 8) {
		if (strcmp(token, "*") == 0) {
			// Wildcard for remaining bits: stop processing further
			// groups.
			break;
		}
...
int main(int argc, char *argv[])
{
	...
	while (fgets(line, sizeof(line), fp)) {
		...
		if (strchr(p, '.')) {
			if (v4_cnt >= 256) {
				fprintf(stderr, "Too many IPv4 entries\n");
				break;
			}
			if (parse_ipv4(p, &v4_list[v4_cnt]) == 0)
				v4_cnt++;
		} else if (strchr(p, ':')) {
			if (v6_cnt >= 256) {
				fprintf(stderr, "Too many IPv6 entries\n");
				break;
			}
			if (parse_ipv6(p, &v6_list[v6_cnt]) == 0)
				v6_cnt++;
		} else {
	...
	for (int i = 0; i < v4_cnt; i++) {
		printf("    { __builtin_bswap32(0x%08x), "
		       "__builtin_bswap32(0x%08x) }, // %u.%u.%u.%u\n",
		       v4_list[i].net, v4_list[i].mask,
		       (v4_list[i].net >> 24) & 0xFF,
		       (v4_list[i].net >> 16) & 0xFF,
		       (v4_list[i].net >> 8) & 0xFF, v4_list[i].net & 0xFF);
	}
	printf("};\n\n");
	// size
