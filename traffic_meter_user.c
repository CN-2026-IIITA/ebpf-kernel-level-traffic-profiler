...
struct traffic_record {
	...
	char dst_ip[INET6_ADDRSTRLEN]; /* Destination IP as string (IPv4 or
					  IPv6) */
	__u8 direction;		       /* 0 = ingress (in), 1 = egress (out) */
	struct timespec
	    timestamp;	 /* Event timestamp with nanosecond precision */
 ...
 * Implement this interface to add new output targets (syslog, socket, database,
 * etc.)
 */
struct output_backend {
	int (*init)(void *config); /* Initialize backend */
	int (*write)(
	    const struct traffic_record *rec); /* Write a traffic record */
...
static int file_write(const struct traffic_record *rec)
{
	...
	fprintf(f, "%s,%u,%s,%s,%ld.%09ld,%llu,%llu\n",
		rec->direction ? "out" : "in", rec->bytes, rec->src_ip,
		rec->dst_ip, rec->timestamp.tv_sec, rec->timestamp.tv_nsec,
 ...
 *        argv[1] = cgroup path (optional, defaults to
 * /sys/fs/cgroup/user.slice) argv[2] = NIC identifier for log filenames
...
int main(int argc, char **argv)
{
	 ...
	 * This contains the BPF programs and maps defined in
	 * traffic_meter.bpf.c
	 */
	struct bpf_object *obj =
	    bpf_object__open_file("traffic_meter.bpf.o", NULL);
	if (!obj) {
	 ...
	 * BPF_F_ALLOW_MULTI allows multiple programs on the same hook (needed
	 * for IPv6).
	 */
	struct bpf_program *prog_egress =
	    bpf_object__find_program_by_name(obj, "traffic_meter_egress");
	if (!prog_egress) {
		fprintf(stderr,
			"Failed to find IPv4 egress program in BPF object\n");
		bpf_object__close(obj);
		close(cgroup_fd);
		g_backend->close();
		return 1;
	...
	if (bpf_prog_attach(prog_egress_fd, cgroup_fd, BPF_CGROUP_INET_EGRESS,
			    BPF_F_ALLOW_MULTI) < 0) {
	...
	struct bpf_program *prog_ingress =
	    bpf_object__find_program_by_name(obj, "traffic_meter_ingress");
	if (!prog_ingress) {
		fprintf(stderr,
			"Failed to find IPv4 ingress program in BPF object\n");
		bpf_prog_detach2(prog_egress_fd, cgroup_fd,
	 ...
	 */
	struct bpf_program *prog_egress_v6 =
	    bpf_object__find_program_by_name(obj, "traffic_meter_egress_v6");
	if (!prog_egress_v6) {
		fprintf(stderr,
			"Failed to find IPv6 egress program in BPF object\n");
		bpf_prog_detach2(prog_egress_fd, cgroup_fd,
				 BPF_CGROUP_INET_EGRESS);
		bpf_prog_detach2(prog_ingress_fd, cgroup_fd,
	 ...
	 */
	struct bpf_program *prog_ingress_v6 =
	    bpf_object__find_program_by_name(obj, "traffic_meter_ingress_v6");
	if (!prog_ingress_v6) {
		fprintf(stderr,
			"Failed to find IPv6 ingress program in BPF object\n");
		bpf_prog_detach2(prog_egress_fd, cgroup_fd,
				 BPF_CGROUP_INET_EGRESS);
		bpf_prog_detach2(prog_ingress_fd, cgroup_fd,
				 BPF_CGROUP_INET_INGRESS);
		bpf_prog_detach2(prog_egress_v6_fd, cgroup_fd,
				 BPF_CGROUP_INET_EGRESS);
		bpf_object__close(obj);
		close(cgroup_fd);
		g_backend->close();
	...
	if (!rb) {
		...
		bpf_prog_detach2(prog_egress_v6_fd, cgroup_fd,
				 BPF_CGROUP_INET_EGRESS);
		bpf_prog_detach2(prog_ingress_v6_fd, cgroup_fd,
				 BPF_CGROUP_INET_INGRESS);
		bpf_object__close(obj);
		close(cgroup_fd);
		g_backend->close();
	 ...
	 * This allows polling both buffers with a single ring_buffer__poll()
	 * call. The callback handle_event_v6() is called for each IPv6 event.
	 */
	if (ring_buffer__add(
		rb, bpf_object__find_map_fd_by_name(obj, "events_v6"),
	...
	printf("eBPF traffic meter loaded (cgroup mode, IPv4+IPv6). Press "
	       "Ctrl+C to exit.\n");
	fflush(stdout);
	 ...
	 * Use bpf_prog_detach2() which takes the program fd for precise
	 * detachment (needed when using BPF_F_ALLOW_MULTI).
	 */
	ring_buffer__free(rb);
